...
func DeconstructPacket(packet *Packet) (*Packet, []forge.Interface) {
	var buffers []forge.Interface
	packet.Data = deconstructData(packet.Data, &buffers)
	packet.Attachments = anvil.New(uint64(len(buffers)))
...
func deconstructData(data any, buffers *[]forge.Interface) any {
	if data == nil {
		return nil
	}
...
func extractBinaryData(data any, buffers *[]forge.Interface) *Placeholder {
...
func deconstructSlice(data []any, buffers *[]forge.Interface) []any {
...
func deconstructMap(data map[string]any, buffers *[]forge.Interface) map[string]any {
...
func ReconstructPacket(packet *Packet, buffers []forge.Interface) (*Packet, error) {
...
func reconstructData(data any, buffers []forge.Interface) (any, error) {
	switch typedData := data.(type) {
	case nil:
		return nil, nil
	case []any:
...
func reconstructSlice(data []any, buffers []forge.Interface) ([]any, error) {
	result := make([]any, 0, len(data))
	for _, item := range data {
...
func reconstructMap(data map[string]any, buffers []forge.Interface) (any, error) {
	if placeholder, err := parsePlaceholder(data); err == nil && placeholder.Placeholder {
		if placeholder.Num < 0 || placeholder.Num >= int64(len(buffers)) {
			return nil, errors.ErrIllegalAttachments
		}
