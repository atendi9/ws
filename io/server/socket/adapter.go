...
func (a *adapter) Rooms() *anvil.Map[Room, *anvil.Set[SocketId]] {
	return a.rooms
}

// Sids returns the map of socket IDs and their associated rooms.
func (a *adapter) Sids() *anvil.Map[SocketId, *anvil.Set[Room]] {
	return a.sids
}
...
func (a *adapter) AddAll(id SocketId, rooms *anvil.Set[Room]) {
	_rooms, _ := a.sids.LoadOrStore(id, anvil.NewSet[Room]())
	for _, room := range rooms.Keys() {
		_rooms.Add(room)
		ids, ok := a.rooms.LoadOrStore(room, anvil.NewSet[SocketId]())
		if !ok {
...
func (a *adapter) _encode(packet *parser.Packet, packetOpts *WriteOptions) []forge.Interface {
	encodedPackets := a.encoder.Encode(packet)

	if len(encodedPackets) == 1 {
		if p, ok := encodedPackets[0].(*forge.String); ok {
			// "4" being the "message" packet type in the Engine.IO protocol
			data := forge.NewFromString("4")
...
func (a *adapter) Sockets(rooms *anvil.Set[Room]) *anvil.Set[SocketId] {
	sids := anvil.NewSet[SocketId]()
...
func (a *adapter) SocketRooms(id SocketId) *anvil.Set[Room] {
	if rooms, ok := a.sids.Load(id); ok {
		return rooms
	}
	return nil
...
func (a *adapter) apply(opts *BroadcastOptions, callback func(*Socket)) {
	...
	if rooms != nil && rooms.Len() > 0 {
		...
		a.sids.Range(func(id SocketId, _ *anvil.Set[Room]) bool {
			if except.Has(id) {
				return true
			}
			if socket, ok := a.nsp.Sockets().Load(id); ok {
...
func (a *adapter) computeExceptSids(exceptRooms *anvil.Set[Room]) *anvil.Set[SocketId] {
	exceptSids := anvil.NewSet[SocketId]()
