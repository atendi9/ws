...
func NewBroadcastOperator(adapter Adapter, rooms *anvil.Set[Room], exceptRooms *anvil.Set[Room], flags *BroadcastFlags) *BroadcastOperator {
...
func (b *BroadcastOperator) Construct(adapter Adapter, rooms *anvil.Set[Room], exceptRooms *anvil.Set[Room], flags *BroadcastFlags) {
	b.adapter = adapter
	if rooms != nil {
		b.rooms = rooms
	}
...
func (b *BroadcastOperator) To(room ...Room) *BroadcastOperator {
	rooms := anvil.NewSet(b.rooms.Keys()...)
	rooms.Add(room...)
...
func (b *BroadcastOperator) Except(room ...Room) *BroadcastOperator {
	exceptRooms := anvil.NewSet(b.exceptRooms.Keys()...)
	...
	timer := chrono.SetTimeout(func() {
		...
		ackOnce.Do(func() {
			if b.flags.ExpectSingleResponse {
				...
				ack(responses.All(), errors.ErrOperationHasTimedOut)
			}
		})
	}, timeout)
		...
		if !timedOut.Load() && expected == actualServerCount.Load() && uint64(responses.Len()) == expectedClientCount.Load() {
			...
			ackOnce.Do(func() {
				if b.flags.ExpectSingleResponse {
					...
					ack(anvil.TryCast[[]any](data), nil)
				} else {
