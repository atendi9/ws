...
type (
	...
	sessionAwareAdapter struct {
		...
		sessions                 *anvil.Map[PrivateSessionId, *SessionWithTimestamp]
		packets                  *anvil.Slice[*PersistedPacket]
		cleanupTimer             *chrono.Timer
	}
)
...
func MakeSessionAwareAdapter() SessionAwareAdapter {
	s := &sessionAwareAdapter{
		Adapter:  MakeAdapter(),
		sessions: &anvil.Map[PrivateSessionId, *SessionWithTimestamp]{},
		packets:  anvil.NewSlice[*PersistedPacket](),
	}
...
func (s *sessionAwareAdapter) Construct(nsp Namespace) {
	...
	s.cleanupTimer = chrono.SetInterval(func() {
...
func (s *sessionAwareAdapter) Close() {
	chrono.ClearInterval(s.cleanupTimer)
	s.Adapter.Close()
}
...
func (s *sessionAwareAdapter) Broadcast(packet *parser.Packet, opts *BroadcastOptions) {
	...
	if isEventPacket && withoutAcknowledgement && notVolatile {
		...
		packet.Data = append(anvil.TryCast[[]any](packet.Data), id)
...
func shouldIncludePacket(sessionRooms *anvil.Set[Room], opts *BroadcastOptions) bool {
