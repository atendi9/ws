...
var (
	reservedEvents    = anvil.NewSet("connect", "connect_error", "disconnect", "disconnecting", "newListener", "removeListener")
	disconnectReasons = anvil.NewSet("transport error", "transport close", "forced close", "ping timeout", "server shutting down", "forced server close")
)
...
type (
	...
	Handshake struct {
		...
		Headers xhttp.IncomingHeaders `json:"headers"`
		...
		// Query parameters
		Query xhttp.ParsedUrlQuery `json:"query"`
	...
	//
	//		chrono.Log().Info(`socket %s connected`, socket.Id())
	//
	...
	//			chrono.Log().Info(`socket %s disconnected due to %s`, socket.Id(), reason[0])
	//		})
	//	})
	Socket struct {
		*StrictEmitter
		...
		server                *Server
		adapter               Adapter
		acks                  *anvil.Map[uint64, Ack]
		fns                   *anvil.Slice[SocketMiddleware]
		flags                 atomic.Pointer[BroadcastFlags]
		_anyListeners         *anvil.Slice[events.Listener]
		_anyOutgoingListeners *anvil.Slice[events.Listener]
...
func MakeSocket() *Socket {
	s := &Socket{
		StrictEmitter: NewStrictEmitter(),

		// Initialize default value
		acks:                  &anvil.Map[uint64, Ack]{},
		fns:                   anvil.NewSlice[SocketMiddleware](),
		_anyListeners:         anvil.NewSlice[events.Listener](),
		_anyOutgoingListeners: anvil.NewSlice[events.Listener](),
		taskQueue:             queue.New(),
	}
...
func (s *Socket) Acks() *anvil.Map[uint64, Ack] {
	return s.acks
}
...
func (s *Socket) Construct(nsp Namespace, client *Client, auth map[string]any, previousSession *Session) {
	...
	if previousSession != nil {
		...
		if client.conn.Protocol() == 3 {
			...
			s.id = SocketId(chrono.New().Generate()) // don't reuse the Engine.IO id because it's sensitive information
		}
		if s.server.Opts().ConnectionStateRecovery() != nil {
			s.pid = PrivateSessionId(chrono.New().Generate())
		}
	}
...
func (s *Socket) buildHandshake(auth map[string]any) *Handshake {
	return &Handshake{
		Headers: anvil.Values(s.Request().Headers().All(), func(value []string) any {
			return value
		}),
		...
		Query: anvil.Values(s.Request().Query().All(), func(value []string) any {
			return value
		}),
		Auth: auth,
	}
...
//		socket.Emit("hello", 1, "2", map[string]any{"3": []string{"4"}, "5": anvil.NewBytesBuffer([]byte{6})})
//
...
func (s *Socket) Emit(ev string, args ...any) error {
	...
	if s.nsp.Server().Opts().ConnectionStateRecovery() != nil {
		...
		s.adapter.Broadcast(packet, &BroadcastOptions{
			Rooms:  anvil.NewSet(Room(s.id)),
			Except: anvil.NewSet[Room](),
			Flags:  &flags,
		})
	} else {
...
func (s *Socket) registerAckCallback(id uint64, ack Ack, timeout *time.Duration) {
	...
	timer := chrono.SetTimeout(func() {

		s.acks.Delete(id)
		ack(nil, errors.ErrOperationHasTimedOut)
	}, *timeout)
	s.acks.Store(id, func(args []any, _ error) {
		chrono.ClearTimeout(timer)
		ack(args, nil)
	})
}
...
func (s *Socket) Join(rooms ...Room) {
	...
	s.adapter.AddAll(s.id, anvil.NewSet(rooms...))
}
	...
	if s.Conn().Protocol() == 3 {
		...
		s.packet(&parser.Packet{
			...
			Data: &struct {
				...
				Pid PrivateSessionId `json:"pid,omitempty"`
			}{
				Sid: s.id,
				Pid: s.pid,
			},
...
func (s *Socket) onack(packet *parser.Packet) {
	if packet.Id != nil {
		if ack, ok := s.acks.Load(*packet.Id); ok {
			...
			ack(anvil.TryCast[[]any](packet.Data), nil)
...
func (s *Socket) _onclose(args ...any) {
	...
	if s.server.Opts().ConnectionStateRecovery() != nil && disconnectReasons.Has(anvil.TryGetAny[string](args, 0)) {

		s.adapter.PersistSession(&SessionToPersist{
			Sid:   s.id,
			Pid:   s.pid,
			Rooms: anvil.NewSet(s.Rooms().Keys()...),
			Data:  s.Data(),
		})
	}
	s._cleanup()
...
func (s *Socket) dispatch(event []any) {
	...
	s.run(event, func(err error) {
		s.Enqueue(func() {
			...
			if s.Connected() {
				s.EmitUntyped(anvil.TryGetAny[string](event, 0), anvil.SubSlice(event, 1)...)
			} else {
...
//	io.On("connection", func(clients ...any) {
//		socket := clients[0].(*socket.Socket)
//		fmt.Println(socket.Rooms()) // *anvil.Set { <socket.id> }
//
//		socket.Join("room1")
//
//		fmt.Println(socket.Rooms()) // *anvil.Set { <socket.id>, "room1" }
//	})
func (s *Socket) Rooms() *anvil.Set[Room] {
	if rooms := s.adapter.SocketRooms(s.id); rooms != nil {
		return rooms
	}
	return anvil.NewSet[Room]()
}
...
func (s *Socket) OffAny(listener events.Listener) *Socket {
	if listener != nil {
		...
		_, _ = s._anyListeners.RangeAndSplice(func(listener events.Listener, i int) (bool, int, int, []events.Listener) {
...
func (s *Socket) OffAnyOutgoing(listener events.Listener) *Socket {
	if listener != nil {
		...
		_, _ = s._anyOutgoingListeners.RangeAndSplice(func(listener events.Listener, i int) (bool, int, int, []events.Listener) {
...
func (s *Socket) newBroadcastOperator() *BroadcastOperator {
	flags := *s.flags.Swap(&BroadcastFlags{})
	return NewBroadcastOperator(s.adapter, anvil.NewSet[Room](), anvil.NewSet(Room(s.id)), &flags)
}
