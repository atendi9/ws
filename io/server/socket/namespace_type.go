...
type Namespace interface {
	...
	Emitter() *StrictEmitter
	Sockets() *anvil.Map[SocketId, *Socket]
	Server() *Server
	Adapter() Adapter
	Name() string
	Ids() uint64
	Fns() *anvil.Slice[NamespaceMiddleware]
...
type ParentNamespace interface {
	...
	Children() *anvil.Set[Namespace]
	CreateChild(string) Namespace
}

// ExtendedError is an alias to xhttp.ExtendedError for backward compatibility
// and convenience in server-side socket code.
type ExtendedError = xhttp.ExtendedError

// NewExtendedError creates a new ExtendedError with the given message and data.
// This is a convenience wrapper around anvil.NewExtendedError.
func NewExtendedError(message string, data any) *ExtendedError {
	return xhttp.NewExtendedError(message, data)
}
